...
def user():
    return User.objects.create_user(
        username="Test_user", email="Test@mail.com", password="Password"
    )
...
class TestProfileUser:
    ...
    def test_profile_page(self, client, user):
        ...
        assert "users/profile.html" in [
            template.name for template in get_response.templates
        ]
...
class TestLoginUser:
    ...
    def test_successful_login(self, client, user):
        response = client.post(
            reverse("users:login"), {"username": "Test_user", "password": "Password"}
        )
...
class TestRegisterUser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"username": "   ", "password": "Password"}, "Обязательное поле"),
            ({"password": "Password"}, "Обязательное поле"),
            ({"username": "Test_user"}, "Обязательное поле"),
        ],
    )
...
class TestUserPasswordChange:
    ...
    def test_view_class_attributes(self):
        ...
        assert UserPasswordChange.success_url == reverse_lazy(
    ...
    @pytest.mark.parametrize(
        "old_pass, new_pass1, new_pass2, errors",
        [
